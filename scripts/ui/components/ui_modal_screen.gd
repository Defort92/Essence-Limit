...
##  - работа во время паузы (PROCESS_MODE_WHEN_PAUSED), скрытие при старте;
...
## Клавиша close_action нажата, когда экран скрыт. По умолчанию ничего не делаем и не
## поглощаем событие — его обработает следующий слушатель (например, HUD). Экран-
## переключатель (экран персонажа) переопределяет это, чтобы открыть себя той же клавишей.
