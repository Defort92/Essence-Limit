...
func apply(target: Node) -> void:
	match effect_type:
		...
		EffectType.BUFF_STRENGTH, EffectType.BUFF_AGILITY, EffectType.BUFF_INTELLECT, EffectType.BUFF_REGEN:
			_apply_buff(target)

func _apply_buff(target: Node) -> void:
	if not target.has_method("apply_timed_modifier"):
		return
	...
	target.apply_timed_modifier(mod, duration, false)
...
func _buff_stat_name() -> String:
	match effect_type:
		...
		EffectType.BUFF_REGEN:     return "regen"
	return ""
