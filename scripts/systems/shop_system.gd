...
## Покупает [param quantity] единиц [param item] у [param vendor].
...
## Продаёт [param quantity] единиц предмета [param item] из рюкзака за золото.
...
func sell(item: ItemData, quantity: int = 1) -> bool:
	if item == null or quantity <= 0 or item.sell_price <= 0:
		return false
	if not InventorySystem.remove_item(item.id, quantity):
		return false
